...
total = pd.read_csv("../GPCR_experimental_structures/binding_energy.tsv", sep='\t')

total['class'] = total['Gprotein_Gene_name'].astype(str).str[:4]  # Divide Galphas by class
total = total[total["class"].isin(("GNAS", "GNAI", "GNAO"))]  # Delete underrepresented classes
mask = total['Gprotein_Gene_name'] == "GNAS"
total.loc[mask, 'class'] = "Gs"
mask = (np.logical_or(total['Gprotein_Gene_name'].astype(str).str[:4] == "GNAI",\
     total['Gprotein_Gene_name'].astype(str).str[:4] == "GNAO"))  # Merge GNAI and GNAO classes
