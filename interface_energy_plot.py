...
sns.set(color_codes=True, style="ticks", rc={'axes.facecolor':'white'})

def draw_violinplot(df, ax, data):  # Draw a violinplot with a p-value bar
...
df.loc[df['class'] == "GNAS", 'class'] = "Gs"
df.loc[df['class'].isin(("GNAI", "GoB", "GNAO")), 'class'] = "Gi/o"  # Merge GNAI and GNAO classes
df = df[df["class"].isin(("Gs", "Gi/o"))]  # Delete underrepresented classes
...
representative = df.sort_values(by=["Resolution", "Gprotein_residues", "Receptor_residues"], \
     key=lambda x: 10**9*df["Resolution"]-10**4*df["Receptor_residues"]-df["Gprotein_residues"])
