...
all_files = glob.glob(path+"/**/*_rmsf.txt", recursive=True)
...
df2=df1.T
path1=('_'.join(path.split('_')[1:5])).strip('/')
df3=pd.read_csv("./plots/violin_Gs_Gi_all_bioalign_"+path1+".tsv", comment="#", sep="\t")
...
df4.to_csv('./plots/'+path1+'_rmsf.tsv',index=False,header=True,sep='\t')
