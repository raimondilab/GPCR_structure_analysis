...
###Adapted from SVDSuperimposer module in Bio.PDB
    ...
    #print (np.sum(diff * diff,1))
    #xyz=(sum(diff * diff))
    rmsf_out=np.sqrt(np.sum(diff * diff,1))
    rmsd_out=np.sqrt(sum(sum(diff * diff)) / coords1.shape[0])
    
    return rmsd_out, rmsf_out
...
####Atom lists for RMSD in place (we don't fit another time)
    ...
    rmsd, rmsf = rmsd_calc(refe_ca_list2, mobi_ca_list2)
    print ("Selection RMSD = ", rmsd)
    #print (rmsf)
    #print (len(rmsf), len(refe_ca_list2), len(mobi_ca_list2))
    ...
    rmsf_file=open("%s_%s_rmsf.txt" % ((cline.refe.split(".cif")[0]).split("/")[-1], (cline.mobi.split(".cif")[0]).split("/")[-1]), "wt")
    for ii in range(len(rmsf)):
        rmsf_file.write("%d\t%f\n" % (ii, rmsf[ii]))