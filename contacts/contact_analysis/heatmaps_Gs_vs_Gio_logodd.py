...
#Plot heatmaps with different filters of contact pairs or GPCR and Gprotein separatley for Gs vs Gio structures and annotated with log odds ratio
...
gpcrsp = gpcrsp[(gpcrsp['Total_count']/len(set(df1['GPCR'])) > 0.1)]
...
gpcrsAp = gpcrsAp[(gpcrsAp['Total_count']/len(set(df1Ap['GPCR'])) > 0.1)]
...
#####Logodds
tf=pd.read_csv("./heatmaps/logs/log_odds_gpcr_pos_all.tsv", comment="#", sep="\t",index_col=0)
tg=pd.read_csv("./heatmaps/logs/log_odds_gprot_pos_all.tsv", comment="#", sep="\t",index_col=0)
tp=pd.read_csv("./heatmaps/logs_pair/log_odds_pairs_all.tsv", comment="#", sep="\t",index_col=0)
...
def plot_clustermap(table,protein=str(),name=str()):    
    ...
    if protein =='gprotein':
        ...
        tg1 = tf.reindex(lr1A)
    elif protein =='gprotein_A':
        tg1 = tg.reindex(lr2A)
    elif protein =='pair':
        tg1 = tp.reindex(lr1gp)
    elif protein =='pair_A':
        tg1 = tp.reindex(lr1gAp)
    elif protein =='pair_1':
        tg1 = ab1.reindex(lr1gp1)
    elif protein =='pair_2':
        tg1 = ab2.reindex(lr1gp2)
    elif protein =='pair_1A':
        tg1 = ab1A.reindex(lr1gp1A)
    tg1.plot.bar(stacked=True, color=['b','r'], edgecolor='black',fontsize=15,width=0.85,legend=False,ax=ax2)
    ax2.set_xticklabels([])
    ax2.set_xlabel('')
    ax2.set_facecolor('white')
    ax2.tick_params(axis=u'both', which=u'both',length=1, direction="inout")
    ax2.hlines(y=0, xmin=-0.5, xmax=len(table.columns)-0.5, linewidth=4, color='black')
    ax2.hlines(y=-5, xmin=-0.5, xmax=len(table.columns)-0.5, linewidth=4, color='black')
    ax2.hlines(y=7, xmin=-0.5, xmax=len(table.columns)-0.5, linewidth=4, color='black')
    ax2.vlines(x=-0.5, ymin=-5, ymax=7, linewidth=4, color='black')
    ax2.vlines(x=len(table.columns)-0.52, ymin=-5, ymax=7, linewidth=4, color='black')
    ax2.set_xlim(-0.5,len(table.columns)-0.5)
    ...
    leg1 = g3.ax_heatmap.legend(hand, lt,frameon=False, ncol=6,loc='lower center',bbox_to_anchor=(1.1,-0.4), bbox_transform=plt.gcf().transFigure,fontsize=80, title='Gprotein family(UCM)/GPCR class/family/structure',title_fontsize=85, columnspacing=0.1, handletextpad=0.1, prop={'weight': 'bold'})
    # gpcr family
    if  'gprotein' in protein:
    ...
    elif   'gpcr' in protein:
        for xtic in g3.ax_heatmap.get_xmajorticklabels():
            if xtic.get_text() in mydict.keys(): # Change color if exist else not
                xtic.set_color(mydict[xtic.get_text()])     
    elif  'pair' in protein:
        for xtic in g3.ax_heatmap.get_xmajorticklabels():
            if xtic.get_text().split('-')[0] in mydict.keys(): # Change color if exist else not
                xtic.set_color(mydict[xtic.get_text().split('-')[0]])            
    g3.savefig('./heatmaps/plots_logs/Gs_vs_Gio_receptor_'+protein+'_cluster_'+name+'.svg',dpi=600,bbox_inches='tight',format='svg')

plot_clustermap(table1,'gprotein','all_logodd')
plot_clustermap(table12,'gpcr','all_logodd')
plot_clustermap(table1A,'gprotein_A','allA_logodd')
plot_clustermap(table12A,'gpcr_A','allA_logodd')
plot_clustermap(table12gp,'pair','01_logodd')
plot_clustermap(table1gAp,'pair_A','01_A_logodd')
...
def plot_clustermap1(table,protein=str(),name=str()):    
    ...
    if protein =='gprotein':
        ...
        tg1 = tf.reindex(lr1gA)
    elif protein =='gprotein_A':
        tg1 = tg.reindex(lr2gA)
    elif protein =='pair_2':
        tg1 = ab2.reindex(lr1gp2)
    elif protein =='pair_2A':
        tg1 = ab2A.reindex(lr1gp2A)
    tg1.plot.bar(stacked=True, color=['b','r'], edgecolor='black',fontsize=15,width=0.85,legend=False,ax=ax2)
    ax2.set_xticklabels([])
    ax2.set_xlabel('')
    ax2.set_facecolor('white')
   ...
   # gfamily and class
    ...
    hand=handles+handles2+handles3+handles1
    lt={**lut,  **gprot_ann, **ann_colors,**lut2}
    leg1 = g3.ax_heatmap.legend(hand, lt,frameon=False, ncol=2,loc='upper center',bbox_to_anchor=(2.85,1.6), bbox_transform=plt.gcf().transFigure,fontsize=80, title='Gprotein family(UCM)/GPCR class/family/structure',title_fontsize=85, columnspacing=0.1, handletextpad=0.1, prop={'weight': 'bold'})
    # gpcr family
   # gpcr family
    if  'gprotein' in protein:
        for xtic in g3.ax_heatmap.get_xmajorticklabels():
...
plot_clustermap1(table1gA,'gprotein_A','02A')
plot_clustermap1(table12gA,'gpcr_A','02A')

#####log abs
ab1=pd.read_csv("./heatmaps/logs_pair/log_odds_pairs_01_abs1.tsv", comment="#", sep="\t",index_col=0)
ab2=pd.read_csv("./heatmaps/logs_pair/log_odds_pairs_01_abs2.tsv", comment="#", sep="\t",index_col=0)

ab11=ab1.reset_index()
comb=ab11[['comb']]
comb01 = comb.merge(df1gcp,left_on=['comb'],right_on=['comb'],how="left")
comb01=comb01.dropna()
comb01.Structural = comb01.Structural.astype("category")
comb01.Structural.cat.set_categories(sorter, inplace=True)
comb01.sort_values(["Structural","comb"],inplace=True)
lr1gp1=comb01["comb"].drop_duplicates().tolist()
table12g1 = pd.pivot_table(comb01 ,values='val', index=['GPCR'],columns=['comb']).fillna(0)
table12g1= table12g1.reindex(columns=lr1gp1)
ab22=ab2.reset_index()
comb2=ab22[['comb']]
comb02 = comb2.merge(df1gcp,left_on=['comb'],right_on=['comb'],how="left")
comb02=comb02.dropna()
comb02.Structural = comb02.Structural.astype("category")
comb02.Structural.cat.set_categories(sorter, inplace=True)
comb02.sort_values(["Structural","comb"],inplace=True)
lr1gp2=comb02["comb"].drop_duplicates().tolist()
table12g2 = pd.pivot_table(comb02 ,values='val', index=['GPCR'],columns=['comb']).fillna(0)
table12g2= table12g2.reindex(columns=lr1gp2)

####
plot_clustermap(table12g1,'pair_1','01_logodd1')
plot_clustermap1(table12g2,'pair_2','01_logodd2')


#####log abs classA
ab1A=pd.read_csv("./heatmaps/logs_pair/log_odds_pair_pos_01_classA_abs1.tsv", comment="#", sep="\t",index_col=0)
ab2A=pd.read_csv("./heatmaps/logs_pair/log_odds_pair_pos_01_classA_abs2.tsv", comment="#", sep="\t",index_col=0)


ab11A=ab1A.reset_index()
combA=ab11A[['comb']]
comb01A = combA.merge(df1gcAp,left_on=['comb'],right_on=['comb'],how="left")
comb01A=comb01A.dropna()
comb01A.Structural = comb01A.Structural.astype("category")
comb01A.Structural.cat.set_categories(sorter, inplace=True)
comb01A.sort_values(["Structural","comb"],inplace=True)
lr1gp1A=comb01A["comb"].drop_duplicates().tolist()
table12g1A = pd.pivot_table(comb01A ,values='val', index=['GPCR'],columns=['comb']).fillna(0)
table12g1A= table12g1A.reindex(columns=lr1gp1A)
ab22A=ab2A.reset_index()
comb2A=ab22A[['comb']]
comb02A = comb2A.merge(df1gcAp,left_on=['comb'],right_on=['comb'],how="left")
comb02A=comb02A.dropna()
comb02A.Structural = comb02A.Structural.astype("category")
comb02A.Structural.cat.set_categories(sorter, inplace=True)
comb02A.sort_values(["Structural","comb"],inplace=True)
lr1gp2A=comb02A["comb"].drop_duplicates().tolist()
table12g2A = pd.pivot_table(comb02A ,values='val', index=['GPCR'],columns=['comb']).fillna(0)
table12g2A= table12g2A.reindex(columns=lr1gp2A)


plot_clustermap(table12g1A,'pair_1A','01A_logodd1')
plot_clustermap1(table12g2A,'pair_2A','01A_logodd2')
